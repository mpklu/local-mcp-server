#!/bin/bash
# Tool: http-client
# Description: HTTP utilities for making requests and API calls
#
# @param function: The function to execute (type: string, required: true)
# @param url: URL to request (type: string, required: false)
# @param method: HTTP method (type: string, required: false)
...
# Tool execution
execute_tool() {
    log_info "Executing http-client with args: $*"
    
    python3 "${TOOL_DIR}/client.py" "$@"
